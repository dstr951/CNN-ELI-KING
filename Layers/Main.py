...
def main():
    ...
    model = Model([
        Conv2D(input_channels=1, output_channels=8, kernel_size=3, stride=1, padding=1),  # Convolution
        ...
        FullyConnected(input_tuple=(15, 120), output_size=128),  # Fully Connected Layer
        FullyConnected(input_tuple=(128,), output_size=10)  # Output Layer (e.g., 10 classes for classification)
    ])
    trained_model = Train.train(model)
    X_validation, Y_validation = Utils.read_test_file(Consts.VALIDATION_PATH)
